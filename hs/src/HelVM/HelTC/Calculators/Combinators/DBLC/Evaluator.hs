...
type ProofValue = ReaderT [Term] (StateT (Map.Map Int (Term, Term)) (State Int))

type Proof = SafeT ProofValue

runProof :: Proof a -> Safe a
  ...
  spine (Level i) as =
    ...
    then do
      ...
      case Map.lookup i tbl of
        Nothing -> liftError $ "Level " <> show i <> " not found in context (whnf)."
...
nf' ee = spine ee [] where
  ...
  spine (Level i) as = do
        tbl <- get
        case Map.lookup i tbl of
          Nothing -> liftError $ "Level " <> show i <> " not found in context."
          Just t  -> spine (fst t) as
...
infer t = do
  ...
  case wt of
    Level i -> do
      tbl <- get
      case Map.lookup i tbl of
           Nothing -> liftError $ "Level " <> show i <> " not found in context durring type inference."
           Just t' -> pure $ snd t'
    Var n -> do
      ctx <- ask
      case (ctx , n) of
        ([], _) -> liftError $ "Cannot infer term variable in empty context."
...
check tr ty =
  case tr of
    Level i -> do
      tbl <- get
      case Map.lookup i tbl of
           Nothing -> liftError $ "Level " <> show i <> " not found in context durring type checking."
           Just (_, t)  -> do
             tnf  <- nf t
             tynf <- nf ty
             if tnf == tynf
             then pure ()
             else liftError $ "Type didn't match durring lookup."
    Var n -> do
      ctx <- ask
      case (ctx , n) of
        ([], _) -> liftError $ "Cannot check type of variable term in an empty context."
        (x:_g, 0) -> do
          xnf  <- nf (quote 0 x)
          tynf <- nf ty
          if tynf == xnf
          ...
          else liftError $ "Term does not have correct type."
    ...
    Lam aty tr' -> do
      ...
      case tyw of
        Lam ty1 ty2 -> do
          ...
          else liftError $ "Type of lam annotation didn't match type annotation."
        U -> do
          check aty U
          local (aty:) $ check tr' U
        _ -> liftError $ "Lambdas can only be Lam or * types."
    tr1 :% tr2 -> do
      ity <- infer (tr1 :% tr2)
      tynf <- nf ty
      itynf <- nf ity
      if tynf == itynf
      then check ty U
      else liftError $ "Failed to unify at application."
    U -> do
      tyw <- nwhnf ty
      case tyw of
        U -> pure ()
        _ -> liftError $  "* can only have type *."
...
checkProgram :: Text -> Proof ()
checkProgram s = go (parseText s [] []) where
  go :: [Term] -> Proof ()
  go (ty:tr:ctx) = do
    check tr ty
    ...
    go ctx
  go (_:[]) = liftError $ "Type is given without implementation."
  go [] = pure ()


evalSource :: BIO m => Text -> m ()
evalSource source = let aaa = source & checkProgram in
  do
    aaa & runProof & liftSafe
    aaa & output & wPutStr
